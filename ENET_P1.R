...
data_ENET_P1 <- read.table("data/ENET01.dat", header=TRUE)
data_ENET_P1

# Missings #
# Declare -999 as missing
data_ENET_P1$event[data_ENET_P1$event == -999] <- NA 
data_ENET_P1
...
# nominal variable):
#
data_ENET_P1$event <- factor(data_ENET_P1$event) 
data_ENET_P1
...
percentile <- 1/3
data_ENET_P1$Time <- data_ENET_P1$Time - quantile(data_ENET_P1$Time, percentile) 
# If you want the variable time to have a 0 value, then you could use:
#data_ENET_P1$Time <- data_ENET_P1$Time - data_ENET_P1$Time[round(length(data_ENET_P1$Time)*percentile)]
#
# Make the quadratic term of time:
data_ENET_P1$sqTime <- data_ENET_P1$Time^2 # quadratic term


# Fit the (unconstrained) MANOVA model #
lm_fit_P1 <- lm(cbind(PCL5, CLL) ~ Time + sqTime + event, data = data_ENET_P1)
#
...
output_P1 <- goric(est, VCOV = VCOV, 
                   hypotheses = list(H_NoTrend = H_NoTrend, 
                                     H_Linear = H_Linear, 
                                     H_Hyperbolic = H_Hyperbolic))
output_P1
#summary(output_P1)
#output_P1$ratio.gw # matrix with ratios of GORICA weights
...
benchmarks_P1 <- benchmark(output_P1, pop_est = my_pop_est, iter = iter)
benchmarks_P1
#
plot(benchmarks_P1, log_scale = T, x_lim = c(0.01,13))$plots[1]
plot(benchmarks_P1, log_scale = T, x_lim = c(0.01,13))$plots[2]
#
# Our findings are extreme / likely when assuming a null population
# or a no-trend population.
# Our findings are quite likely in a linear-trend population with
est_lin[c(2,6)]
# and in a population based on our MANOVA estimates.
