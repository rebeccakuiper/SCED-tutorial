...
data_ENET_P10 <- read.table("data/ENET10.dat", header=TRUE)
data_ENET_P10

# Missings #
# Declare -999 as missing
data_ENET_P10$event[data_ENET_P10$event == -999] <- NA 
data_ENET_P10
...
# nominal variable):
#
data_ENET_P10$event <- factor(data_ENET_P10$event) 
data_ENET_P10
...
percentile <- 1/3
data_ENET_P10$Time <- data_ENET_P10$Time - quantile(data_ENET_P10$Time, percentile) 
# If you want the variable time to have a 0 value, then you could use:
#data_ENET_P10$Time <- data_ENET_P10$Time - data_ENET_P10$Time[round(length(data_ENET_P10$Time)*percentile)]
#
# Make the quadratic term of time:
data_ENET_P10$sqTime <- data_ENET_P10$Time^2 # quadratic term


# Fit the (unconstrained) MANOVA model #
lm_fit_P10 <- lm(cbind(PCL5, CLL) ~ Time + sqTime + event, data = data_ENET_P10)
#
...
output_P10 <- goric(est, VCOV = VCOV, 
                   hypotheses = list(H_NoTrend = H_NoTrend, 
                                     H_Linear = H_Linear, 
                                     H_Hyperbolic = H_Hyperbolic))
output_P10
#summary(output_P10)
#output_P10$ratio.gw # matrix with ratios of GORICA weights
