...
data_ENET_P3 <- read.table("data/ENET03.dat", header=TRUE)
data_ENET_P3

# Missings #
# Declare -999 as missing
data_ENET_P3$event[data_ENET_P3$event == -999] <- NA 
data_ENET_P3
...
# nominal variable):
#
data_ENET_P3$event <- factor(data_ENET_P3$event) 
data_ENET_P3
...
percentile <- 1/3
data_ENET_P3$Time <- data_ENET_P3$Time - quantile(data_ENET_P3$Time, percentile) 
# If you want the variable time to have a 0 value, then you could use:
#data_ENET_P3$Time <- data_ENET_P3$Time - data_ENET_P3$Time[round(length(data_ENET_P3$Time)*percentile)]
#
# Make the quadratic term of time:
data_ENET_P3$sqTime <- data_ENET_P3$Time^2 # quadratic term


# Fit the (unconstrained) MANOVA model #
lm_fit_P3 <- lm(cbind(PCL5, CLL) ~ Time + sqTime + event, data = data_ENET_P3)
#
...
output_P3 <- goric(est, VCOV = VCOV, 
                   hypotheses = list(H_NoTrend = H_NoTrend, 
                                     H_Linear = H_Linear, 
                                     H_Hyperbolic = H_Hyperbolic))
output_P3
#summary(output_P3)
#output_P3$ratio.gw # matrix with ratios of GORICA weights
...
benchmarks_P3 <- benchmark(output_P3, pop_est = my_pop_est, iter = iter)
benchmarks_P3
#
plot(benchmarks_P3, log_scale = T, x_lim = c(0.01,13))$plots[1]
plot(benchmarks_P3, log_scale = T, x_lim = c(0.01,13))$plots[2]
#
# Our findings are likely when assuming a null population
# or a no-trend population.
# Our findings are unlikely/extreme in a linear-trend population with
est_lin[c(2,6)]
