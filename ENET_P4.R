...
data_ENET_P4 <- read.table("data/ENET04.dat", header=TRUE)
data_ENET_P4

# Missings #
# Declare -999 as missing
data_ENET_P4$event[data_ENET_P4$event == -999] <- NA 
data_ENET_P4
...
# nominal variable):
#
data_ENET_P4$event <- factor(data_ENET_P4$event) 
data_ENET_P4
...
percentile <- 1/3
data_ENET_P4$Time <- data_ENET_P4$Time - quantile(data_ENET_P4$Time, percentile) 
# If you want the variable time to have a 0 value, then you could use:
#data_ENET_P4$Time <- data_ENET_P4$Time - data_ENET_P4$Time[round(length(data_ENET_P4$Time)*percentile)]
#
# Make the quadratic term of time:
data_ENET_P4$sqTime <- data_ENET_P4$Time^2 # quadratic term


# Fit the (unconstrained) MANOVA model #
lm_fit_P4 <- lm(cbind(PCL5, CLL) ~ Time + sqTime + event, data = data_ENET_P4)
#
...
output_P4 <- goric(est, VCOV = VCOV, 
                   hypotheses = list(H_NoTrend = H_NoTrend, 
                                     H_Linear = H_Linear, 
                                     H_Hyperbolic = H_Hyperbolic))
output_P4
#summary(output_P4)
#output_P4$ratio.gw # matrix with ratios of GORICA weights
