...
data_ENET_P5 <- read.table("data/ENET05.dat", header=TRUE)
data_ENET_P5

# Missings #
# Declare -999 as missing
data_ENET_P5$event[data_ENET_P5$event == -999] <- NA 
data_ENET_P5
...
# nominal variable):
#
data_ENET_P5$event <- factor(data_ENET_P5$event) 
data_ENET_P5
...
percentile <- 1/3
data_ENET_P5$Time <- data_ENET_P5$Time - quantile(data_ENET_P5$Time, percentile) 
# If you want the variable time to have a 0 value, then you could use:
#data_ENET_P5$Time <- data_ENET_P5$Time - data_ENET_P5$Time[round(length(data_ENET_P5$Time)*percentile)]
#
# Make the quadratic term of time:
data_ENET_P5$sqTime <- data_ENET_P5$Time^2 # quadratic term


# Fit the (unconstrained) MANOVA model #
lm_fit_P5 <- lm(cbind(PCL5, CLL) ~ Time + sqTime + event, data = data_ENET_P5)
#
...
output_P5 <- goric(est, VCOV = VCOV, 
                   hypotheses = list(H_NoTrend = H_NoTrend, 
                                     H_Linear = H_Linear, 
                                     H_Hyperbolic = H_Hyperbolic))
output_P5
#summary(output_P5)
#output_P5$ratio.gw # matrix with ratios of GORICA weights
